...
void MenuState::handleInput(sf::Event& event) {
    if (event.type == sf::Event::KeyPressed) {
        if (event.key.code == sf::Keyboard::Up) {
            ...
            if (selectedIndex == 0) manager.setState(std::make_unique<GameState>(manager));
            else if (selectedIndex == 1) manager.setState(std::make_unique<RulesState>(manager));
            else if (selectedIndex == 2) manager.setState(std::make_unique<SettingsState>(manager));
            else if (selectedIndex == 3) manager.setState(nullptr);
        }
    }
}
...
void MenuState::draw(sf::RenderWindow& window) {
    for (const auto& text : options) window.draw(text);
}
